...
class SolicitudArriendo(models.Model):
    ...
    arrendatario = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='solicitudes')
    inmueble = models.ForeignKey(Inmueble, on_delete=models.CASCADE)
    fecha_solicitud = models.DateTimeField(auto_now_add=True)
    estado_solicitud = models.CharField(max_length=10, choices=ESTADO_SOLICITUD_CHOICES, default='pendiente')

    def __str__(self):
        return f'Solicitud de {self.arrendatario.user.first_name} {self.arrendatario.user.last_name} para {self.inmueble.nombre}'
